...
import { verifyNexusAccess } from './nexus-core.js';
...
// ─── NexusGuard ───────────────────────────────────────────────────────────────
// recheckInterval: segundos entre re-verificaciones en background (default 30s)
export function NexusGuard({ children, recheckInterval = 30 }) {
  ...
  // Re-verificación periódica en background
  useEffect(() => {
  ...
  // Interceptar fetch global: si cualquier respuesta es 401/403,
  // disparar re-verificación inmediata para bloquear la sesión al instante
  useEffect(() => {
    const origFetch = window.fetch;
    window.fetch = async (...args) => {
      const res = await origFetch(...args);
      // Ignorar peticiones al propio endpoint de verificación
